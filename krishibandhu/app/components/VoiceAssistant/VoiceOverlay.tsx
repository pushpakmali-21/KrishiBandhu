...
export const VoiceOverlay: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-white dark:bg-gray-800 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in duration-300 border border-transparent dark:border-gray-700">
        
        {/* Header with close button */}
        <div className="flex justify-between items-center p-6 border-b border-gray-100 dark:border-gray-700">
          <h2 className="text-lg font-black text-gray-800 dark:text-white">Voice Assistant</h2>
          <button
            onClick={() => {
              resetTranscript();
              stopListening();
            }}
            className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transitions-colors"
        ...
        <div className="p-8 text-center space-y-6">
          ...
          <div>
            {isProcessing ? (
              ...
              <div className="flex justify-center">
                <div className="w-24 h-24 flex items-center justify-center bg-green-100 dark:bg-green-900/30 rounded-full">
          ...
          {/* Transcript display */}
          <div className="min-h-16 bg-gray-50 dark:bg-gray-900/50 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
            {transcript ? (
              <p className="text-base font-semibold text-gray-800 dark:text-gray-200 leading-relaxed">&quot;{transcript}&quot;</p>
            ) : (
              <p className="text-sm text-gray-400 dark:text-gray-500 italic">Say something...</p>
            )}
          </div>

          {/* Last response display */}
          {lastResponse && !isProcessing && !isListening && (
            <div className="min-h-16 bg-green-50 dark:bg-green-950/20 rounded-2xl p-4 border border-green-100 dark:border-green-800 animate-in slide-in-from-bottom-2 duration-300">
              <p className="text-sm font-medium text-green-800 dark:text-green-300 leading-relaxed">{lastResponse}</p>
            </div>
          )}
          ...
          <div className="flex justify-center">
            <div className="inline-flex items-center gap-2 bg-blue-100 dark:bg-blue-900/30 px-4 py-2 rounded-full">
              ...
              <span className="text-xs font-semibold text-blue-600 dark:text-blue-400">{langNames[language]}</span>
            </div>
          </div>
            ...
            {lastResponse && !isProcessing && !isListening && (
              <>
                <button
                  ...
                  className="flex-1 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 px-6 py-3 rounded-xl font-bold text-sm hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors"
                >
                  Close
                </button>
                <button
